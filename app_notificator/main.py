...
def get_provider(card_name):
    """Возвращает провайдера (prov.prov), к которому относится карта."""
    try:
...
def get_tg_settings(provider):
    ...
    if not all([token, chat_id]):
        logger.error(
            f"Не найдены переменные TG_TOKEN_{provider} и/или TG_CHAT_ID_{provider}"
        )
        return None
...
def send_message(card_name, message):
    ...
    if not provider:
        logger.error(f"Не удалось определить провайдера для карты {card_name}")
        return
...
def handle_post():
    try:
        ...
        send_message(card_name, make_message(card_name, number, text))
